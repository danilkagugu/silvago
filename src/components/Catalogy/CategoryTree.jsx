          ...
          sortCategories(categoriesTorgsoft).map((category,index) => (
            ...
            >
              <div className={css.productMenuTitle}> 
                <Link className={css.productsMenuTitleLink} to={`/${category.slug}`}>
                {category.name}
              </Link></div>
             
              {category.children.length > 0 && (
                <div className={`${css.subMenu}`} >
                  <ul className={css.subMenuList} ref={(el) => (subMenuRefs.current[index] = el)}>
                    {sortCategories(category.children).map((child) => (
                      ...
                      >
                        ...
                        >
                          <span className={css.subMenuTitleSpan}>{child.name}</span>
