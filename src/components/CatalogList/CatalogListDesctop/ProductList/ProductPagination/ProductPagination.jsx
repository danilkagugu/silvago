...
const ProductPagination = ({ currentPage = 1, totalPages = 1, handlePageChange }) => {
  if (totalPages <= 1) return null;
      ...
      <Pagination
        ...
        renderItem={(item) => (
          <PaginationItem
            {...item}
            components={{
              previous: IoChevronBackSharp,
              next: IoChevronForward,
            }}
            className={css.pageItem}
          />
        )}
        siblingCount={2} // Кількість сторінок, що відображаються поруч із активною
        boundaryCount={1} // Кількість сторінок на початку та в кінці списку
        showFirstButton
        showLastButton
